    ...
    module InstanceMethods
      ...
      def render_locomotive_page
        if request.fullpath =~ /^\/admin\//
          ...
          current_path  = ("/" + (params[:path] || request.fullpath).split('?').first).gsub(/\/+/, '/')
          redirect_to(@page.canonical_url) and return if !@page.canonical_url.blank? && current_path != @page.canonical_url
